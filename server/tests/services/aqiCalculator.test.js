...
describe('Service: aqiCalculator', () => {
    ...
    describe('calculateEAQI()', () => {
        it('должен возвращать максимальный под-индекс (наихудший показатель) по всем загрязнителям', () => {
            // Передаем концентрации: pm25, pm10, no2, o3, so2
            // Для теста зададим значения, где pm10 будет самым грязным показателем.
            // pm25: 15 (индекс ~30), pm10: 45 (индекс ~90), no2: 10, o3: 20, so2: 5
            const overallAqi = calculateEAQI(15, 45, 10, 20, 5);

            // pm10 концентрация 45 попадает в интервал 40-50 (индексы 40-60).
            // (45 - 40) / (50 - 40) * (60 - 40) + 40 = 5 / 10 * 20 + 40 = 50.
            // Поскольку 70 - наибольшее значение, метод должен вернуть его.
